  ...
  head: () => ({
    meta: [
      { title: "Listing detail · Ivy Homes" },
      {
        name: "description",
        content:
          "View property details, pricing, area, and seller information for a Gurgaon listing.",
      },
...
function ListingDetail() {
  ...
  const toggleSaved = async () => {
    try {
      await ivyRequest(`/v1/saved${saved ? `/${encodeURIComponent(id)}` : ""}`, session, {
        ...
        body: saved ? null : JSON.stringify({ listing_id: id }),
      });
  ...
  return (
    <PageFrame session={session} active="listings">
      <div className="detail-hero">
        ...
        <div className="detail-copy">
          <Link to="/" search={{ view: "listings" }} className="text-link">
            <ArrowLeft size={14} /> Back to homes
          </Link>
          <p className="eyebrow" style={{ marginTop: 28 }}>
            {titleCase(listing.property_type)} · {listing.bedroom} bedroom
          </p>
          <h1>{listing.apartment_name}</h1>
          <p className="intro-copy">
            <MapPin size={15} /> {titleCase(listing.locality)}, Gurgaon
          </p>
          <div className="detail-specs">
            <div className="detail-spec">
              <span>Carpet area</span>
              <strong>{listing.carpet_area.toLocaleString("en-IN")} sq ft</strong>
            </div>
            <div className="detail-spec">
              <span>Bathrooms</span>
            ...
            <div className="detail-spec">
              ...
              <strong>{titleCase(listing.furnishing)}</strong>
            </div>
            <div className="detail-spec">
              <span>Listed</span>
            ...
            <div className="detail-spec">
              ...
              <strong>{listing.is_verified ? "Verified" : "Unverified"}</strong>
            </div>
          </div>
          <Button onClick={toggleSaved} variant={saved ? "secondary" : "default"}>
      ...
      <section className="detail-description">
        ...
        <div className="card-foot">
          <span>
            {listing.posted_by === "owner" ? "Listed by owner" : `Listed by ${listing.posted_by}`}
          </span>
          <span>
            <CheckCircle2 size={14} /> {listing.posted_by_name}
          </span>
          <span>
            <Share2 size={14} /> {listing.listing_id}
          </span>
        </div>
      </section>
    </PageFrame>
    ...
    <div className="login-page">
      <div className="login-story">
        ...
        <div className="story-copy">
          <p className="eyebrow">Private property search</p>
          <h1>Find your next place in Gurgaon.</h1>
          <p>Sign in to browse live listings and keep your shortlist with you.</p>
        </div>
      </div>
      <div className="login-panel">
        <div className="login-form">
          <h2>Sign in first</h2>
          <p>Your saved homes and property details are available after you sign in.</p>
